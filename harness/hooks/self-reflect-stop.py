...
def main() -> int:
    ...
    reflect_marker = root / ".harness-reflect"
    if not reflect_marker.is_file():
        return 0

    # 验证 marker 是由当前 session 创建的
    # marker 文件内容应该包含创建它的 session_id
    try:
        marker_content = reflect_marker.read_text("utf-8").strip()
        if marker_content != session_id:
            # marker 是其他 session 创建的，忽略
            return 0
    except Exception:
        # 无法读取 marker，保守起见放行
        return 0
