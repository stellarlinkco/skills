...
def main() -> int:
    ...
    if not _is_harness_active(root):
        return 0
    hc.maybe_log_hook_event(root, payload, Path(__file__).name)
    ...
    if not pending_eligible and not retryable and not in_progress_blocking:
        ...
        # Write session_id to marker so self-reflect can verify it's from current session
        try:
            session_id = payload.get("session_id", "")
            (root / ".harness-reflect").write_text(session_id, encoding="utf-8")
        except Exception:
            pass
        try:
