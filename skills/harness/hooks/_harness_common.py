...
def read_hook_payload() -> dict[str, Any]:
    """Read JSON payload from stdin (sent by Codex to command hooks)."""
    raw = sys.stdin.read()
    if not raw.strip():
        return {}
    try:
...
def maybe_log_hook_event(root: Path, payload: dict[str, Any], hook_script: str) -> None:
    ...
    Call this only after the .harness-active guard passes (so hooks don't "work"
    when harness isn't in use).
    """
